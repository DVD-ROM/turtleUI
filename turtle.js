...
function svgClose() {
  ...
  svgClosePath();
  var svgOpenBlob =
    '<svg id="turtle-svg" xmlns="http://www.w3.org/2000/svg" version="1.1" width="' +
...
/*******************************************************************************
 * setSize -- sets the canvas dimensions in pixels, erases everything and resets turtle to
 *            center facing up with pen down
 * added by David Huang
 * arguments:
 *    width: desired width of canvas in pixels
 *    height: desired height of canvas in pixels
 * returns: None
 * effects: erases the canvas
 ******************************************************************************/
function setSize(width, height) {
  ...
  setAttributesOfElementWithID("imagecanvas", dimensions);
  setAttributesOfElementWithID("turtlecanvas", dimensions);
  showturtle();
  home();
  pendown();
}
...
/*******************************************************************************
 * setAttributesOfElementWithID -- gives desired attributes to the element with matching ID
 * arguments:
 *    elementID: id of element to give attributes to
 *    attributes: JS object where each key's name is an HTML attribute name and the value is the desired value
 * returns: None
 ******************************************************************************/
function setAttributesOfElementWithID(elementID, attributes) {
...
function fillshape(styl) {
  if (turtle.shape) {
    ...
    if (typeof styl === "number") {
      if (styl < 16) {
        // assume standard logo turtle color
...
function circle(radius, extent, CW) {
  ...
  if (extent === undefined) {
    ...
    svgBlob =
      svgBlob +
      '<circle cx="' +
      round(turtle.pos.x, svgPrecision) +
      '" cy="' +
      round(turtle.pos.y, svgPrecision) +
      '" r="' +
      round(radius, svgPrecision) +
      '"' +
      ' style="stroke:' +
      turtle.color +
      "; stroke-width:" +
...
function restoreTurtleState(tState) {
  ...
  console.log(
    "rTS font: " + imageContext.font + " color:" + imageContext.strokeStyle
  );
}
